...
\begin{tabularx}{\textwidth}{p{2.1cm}>{\arraybackslash}X}
  \bfseries{Languages} & Python$^+$ $\cdot$ R Tidyverse $\cdot$ Espa\~nol$^+$ $\cdot$ English$^+$ $\cdot$ Italiano \\  % $\cdot$ SAS   $\cdot$ BASH $\cdot$ RegExp $\cdot$ ECL 
  \bfseries{Tools} & PyTorch, Keras, Tensorflow  $\cdot$ Pandas$^+$, \mbox{MongoDB}, PostgreSQL $\cdot$ SciPy$^+$ $\cdot$ Ray, PySpark, Hadoop $\cdot$ Plotly Dash, Flask  $\cdot$ BeautifulSoup, Scrapy $\cdot$ Docker \\% $\cdot$ GraphViz $\cdot$ vim  $\cdot$ SSH   \\
...
\textbf{Lead Machine Learning Engineer}, \textit{Dendra} \hfill 2020 \\
Using machine learning for ecosystem restoration.
