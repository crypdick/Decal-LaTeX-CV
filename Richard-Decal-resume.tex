...
\begin{tabularx}{\textwidth}{p{3cm}>{\arraybackslash}X}
  \bfseries{Algorithms} & Supervised, unsupervised, and reinforcment learning $\cdot$ Cross-validation    $\cdot$  PCA $\cdot$ \mbox{Neural style transfer$^\star$} \\
  \bfseries{Languages} & Python$^\star$   $\cdot$ some C++  \\
  \bfseries{Coding} & Tensorflow  $\cdot$ Pandas$^\star$ $\cdot$ Numpy$^\star$ $\cdot$ Scikit-learn   $\cdot$ \mbox{Matplotlib  } $\cdot$ Git$^\star$ $\cdot$ \mbox{Unix CLI*}  \\
  %\bfseries{OS} & Linux$^\star$ $\cdot$ MS Windows$^\star$ $\cdot$  Mac OS X$^\star$ $\cdot$ Android$^\star$ $\cdot$ iOS  \\
  \bfseries{Research} &  Experienced in computational neuroscience, biochemistry, genetics research. I am co-author on several peer-reviewed scientific publications.\\
  \bfseries{Misc.} & Espa\~nol$^\star$ $\cdot$ English$^\star$ $\cdot$ Italian  $\cdot$ Technical writing  \\  % Web data scraping USA/EU Dual-citizen
  \bfseries{Passions} & Travel, teaching, learning, social justice, climate change, tiny living. \\
\end{tabularx}
...
\begin{tabularx}{\textwidth}{p{3cm}>{\arraybackslash}X}
\bfseries{\href{https://github.com/crypdick/policy-gradients-cartpole}{Solving \mbox{CartPole}}} &  \tabitem I solved \href{https://gym.openai.com/envs/CartPole-v0}{CartPole}, a continuous classic control video game, using reinforcement learning and policy gradients.
\end{tabularx}

\begin{tabularx}{\textwidth}{p{3cm}>{\arraybackslash}X}
\bfseries{\href{https://github.com/crypdick/predict-donors-California-census}{Predicting Californian Incomes}} &  \tabitem Data analysis of the \href{https://www.aaai.org/Papers/KDD/1996/KDD96-033.pdf}{California census data} to predict incomes of individuals based on 14 characteristics using feature engineering, support vector machines, and cross-validation.
\end{tabularx}
