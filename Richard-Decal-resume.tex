...
\begin{tabularx}{\textwidth}{p{3cm}>{\arraybackslash}X}
  \bfseries{Languages} & Python$^+$   $\cdot$ R $\cdot$ SQL $\cdot$ BASH $\cdot$ Espa\~nol$^+$ $\cdot$ English$^+$ $\cdot$ Italiano \\
  \bfseries{Packages} & Tensorflow  $\cdot$ Pandas$^+$ $\cdot$ Numpy$^+$ $\cdot$ Scikit-learn   $\cdot$ \mbox{Matplotlib}$^+$   $\cdot$ Tidyverse\\
  %\bfseries{OS} & Linux$^\star$ $\cdot$ MS Windows$^\star$ $\cdot$  Mac OS X$^\star$ $\cdot$ Android$^\star$ $\cdot$ iOS  \\
  \bfseries{Passions} & Travel, teaching, learning, social justice, my tiny home. Research interests: molecular biology, climate science, computational neuroscience.
\end{tabularx}
...
\item Teaching Python, increasing student engagement, and goal planning. \\
\end{itemize}

\ressubheading{Research Assistant}{Seattle, WA}{Fairhall Lab, Dept. of Biophysics, Uni. of Washington}{2014 --- 2016}
\begin{itemize}[noitemsep,topsep=0pt,parsep=0pt,partopsep=0pt, nolistsep] \vspace{-10pt}
\item Iteratively developed dynamical agent-based models of mosquito thermonavigation and benchmarked them against experimental data.
...
\resheading{~~Publications}
      ...
      \bfseries{\href{https://github.com/crypdick/RoboSkeeter}{Roboskeeter}}  \mbox{\textnormal{\textit{(ongoing)}}} &  Modelling mosquito heat-tracking behavior from windtunnel experiments.
      \end{tabularx}
