...
\begin{tabularx}{\textwidth}{p{2.1cm}>{\arraybackslash}X}
  ...
  \bfseries{Tools} & Tensorflow  $\cdot$ Pandas$^+$ $\cdot$ Numpy$^+$ $\cdot$ PySpark $\cdot$ Hadoop $\cdot$ Scikit-learn   $\cdot$ \mbox{Matplotlib}$^+$   $\cdot$ Tidyverse $\cdot$ SQL $\cdot$ \mbox{MongoDB} $\cdot$ HBase $\cdot$ HPCC\\
