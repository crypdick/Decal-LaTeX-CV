...
\begin{tabularx}{\textwidth}{p{2.1cm}>{\arraybackslash}X}
  \bfseries{Languages} & Python$^+$ $\cdot$ R  $\cdot$ BASH $\cdot$ ECL $\cdot$ Espa\~nol$^+$ $\cdot$ English$^+$ $\cdot$ Italiano \\  % $\cdot$ SAS
  \bfseries{Tools} & Tensorflow  $\cdot$ Pandas$^+$ $\cdot$ SciPy$^+$ $\cdot$ PySpark $\cdot$ Flask $\cdot$ Hadoop  $\cdot$ Tidyverse $\cdot$ PostgreSQL $\cdot$ \mbox{MongoDB} $\cdot$ HBase $\cdot$ HPCC $\cdot$ SSH\\
...
\textbf{Data Scientist}, \textit{Florence A. Rothman Institute} \hfill 2018\\
Studying effect of chronic conditions on heart failure with autoencoders, clustering, \& Markov models.\\

\textbf{Research Intern}, \textit{Allen Institute for Brain Science} \hfill Summer 2018\\
Applying deep reinforcement learning for tracing neurons on petabytes of brain microscope data.\\

%\ressubheading{Classroom Mentor}{Remote}{Udacity.com, Intro to Programming Nanodegree}{2017 --- Now}
%\begin{itemize}[noitemsep,topsep=0pt,parsep=0pt,partopsep=0pt, nolistsep] \vspace{-10pt}
%\item I provide tailored guidance for students learning Python and data analysis. \\
%\end{itemize}
...
Simulating mosquito thermal plume navigation with agent-based models of windtunnel experiments.\\
