...
\begin{tabularx}{\textwidth}{p{2.1cm}>{\arraybackslash}X}
  ...
  \bfseries{Tools} & Tensorflow  $\cdot$ Pandas$^+$ $\cdot$ SciPy$^+$ $\cdot$ PySpark $\cdot$ Flask $\cdot$ Hadoop  $\cdot$ Tidyverse $\cdot$ \mbox{MongoDB} $\cdot$ PostgreSQL $\cdot$ HBase $\cdot$ HPCC $\cdot$ SSH\\
